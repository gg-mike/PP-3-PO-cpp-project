...
struct Log {
	Log(int ID, std::string cityA, std::string cityB, float duration, float cost, char type)
		: ID(ID), cityA(cityA), cityB(cityB), duration(duration), cost(cost), type(type) {}
	Log() : ID(-1), cityA(""), cityB(""), cityA_ID(-1), cityB_ID(-1), duration(-1), cost(-1), type('\0') {}
